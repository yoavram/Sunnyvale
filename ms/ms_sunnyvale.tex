...
Important modes of cultural transmission are vertical (from parent to offspring), horizontal (between peers), and oblique (from adults in the parental generation to offspring) transmission. 
...
% TODO what can of spatial differences can we imagine in transmission?
% two populations, one has first cousin marriages and one doesn't (endogmia)
% orthodox religious communities
% boarding schools for one population
% foraging in groups or families
% pigmies women 'migration' - adoption by husband family
% interesting: migration correlated with transmission rate, future work

% matrilineal primates - vertical transmission stronger in females than males
% similar differences in ultra religious human societies
...
Thus for a protected polymorphism, we require
% TODO check RHS
\begin{equation}
1 < F'_A(1) F'_B(1) = \Big(1-\rho_A\frac{s}{1+s}\Big)\Big(1+\rho_B s\Big) 
= 1+\frac{s}{1+s}\big[\rho_B(1+s-\rho_A)-\rho_A\big],
\end{equation}
and
\begin{equation}
...
\paragraph{Examples.}
First, if $s_t$ and $\rho_t$ are independent ($cov(s_t, \rho_t)=0$) and $s_t$ is symmetric around zero, then $E[z_t]=0$ and $E[-z_t/(1+s_t)]>0$ (because $E[s_t/(1+s_t)] < E[s_t]$).
...
Therefore, if we take the i.i.d fitness random variables for $A$ and $B$ to be $w_{A,t}, w_{B,t} \sim U(0,1)$, respectively, and define $s_t=(w_{A,t}-w_{B,t})/w_{B,t}$, then neither $A$ nor $B$ has an advantage, on average (i.e. $E[w_{A,t}/w_{B,t}]=E[w_{B,t}/w_{A,t}]$, see \autoref{fig:beta}A), and both $E[z_t]$ and $E[-z_t/(1+s_t)]$ are positive, so that both fixations are not stochastically locally stable, and we expect the population to converge to a polymorphic distribution.
...
We now describe a model in which fluctuations in selection and transmission occur in space, rather then time; we model a population divided to two demes.
...
For the simple case of symmetric migration $m_1=m_2=m<1/2$ occurring after symmetric selection $w_1=1/w_2=w>1$, the recursions~\eqref{eq:migration_model} become
...
When $\rho_1=1$ and $\rho_2=\rho$, the stability of $(0,0)$ (i.e. fixation in $B$) is determined by the characteristic polynomial $f(\lambda)$ with coefficients
\begin{equation}
\begin{aligned}
a &= 1, \quad
...
$$
\Delta = (1 - m)^2 (w + 1-\rho + \rho/w)^2 - 4(1-2m)(w(1-\rho)+\rho),
$$
which is positive for $m=0$ and $m=1/2$:
\begin{equation}\begin{aligned}
\Delta(m=0) = \Big(\frac{(w + \rho) (w-1)}{w}\Big)^2 \\
\Delta(m=1/2) = \Big(\frac{w^2 + w(1-\rho) + \rho}{2 w}\Big)^2.
\end{aligned}\end{equation}
In addition, if we write $\Delta$ as a polynomial in $m$, its discriminant is negative $\big(- 16 (w + \rho)^2 (w - 1)^2 \big(w (1 - \rho) + \rho \big) / w^2\big)$, and therefore the discriminant $\Delta$ is positive for any $0 \le m \le 1/2$ and $f(\lambda)$ has two real roots. \\
Second, $a>0$, $b<0$, and $c>0$ and stability can be analyzed by the sign of $a+b+c$.
...
The stability of $(1,1)$ (i.e. fixation in $A$) is determined by the characteristic polynomial $f(\lambda)$ with coefficients
\begin{equation}
\begin{aligned}
a &= 1, \quad
b &= -(1 - m)(w \rho + 1 - \rho + 1/w), \quad
c &= (1 - 2m) \big((1-\rho)/w + \rho\big),
\end{aligned}
\end{equation}
First, the discriminant is 
$$
\Delta = (1 - m)^2(w \rho + 1 - \rho + 1/w)^2 - 4 (1 - 2m) \big((1-\rho)/w + \rho\big),
$$
which is positive for $m=0$ and $m=1/2$:
\begin{equation}\begin{aligned}
\Delta(m=0) = \Big(\frac{(w \rho + 1) (w-1)}{w}\Big)^2 \\
\Delta(m=1/2) = \Big(\frac{w^2 \rho + w(1-\rho) + 1}{2 w}\Big)^2.
\end{aligned}\end{equation}
In addition, if we write $\Delta$ as a polynomial in $m$, its discriminant is negative $\big(- 16 (w\rho + 1)^2 (w - 1)^2 \big(\rho w + 1 - \rho \big) / w^3\big)$, and therefore the discriminant $\Delta$ is positive for any $0 \le m \le 1/2$ and $f(\lambda)$ has two real roots. \\
Second, $a>0$, $b<0$, and $c>0$, and if $\rho=1$ then $a+b+c<0$, as before.
...
\begin{equation} \label{eq:migration_model_unconditional_symmetric_condition_m2}
m > \hat m = \frac{\rho (w-1)}{\rho (w-1) +1-\rho}.
\end{equation}
In addition, $2a+b$ is linear in $m$ and equal to 2 when $m=1$; it is equal to zero when $m=(w-1)(w\rho-1)/(w(w-1)\rho+w+1) < \hat m$, and therefore (\ref{eq:migration_model_unconditional_symmetric_condition_m}, also \ref{eq:migration_model_unconditional_symmetric_condition_m2})
is sufficient for stability of fixation of $A$.

\paragraph{Examples.}

\autoref{fig:migration_rho} and \autoref{fig:migration_m} show the stable frequencies of phenotype $A$~ (eq.~\ref{eq:migration_model_unconditional_symmetric_polymorphism}) and the stable population mean fitnesses in the two demes with symmetric selection where $A$ is favored in deme 1 and $B$ is favored in deme 2 with similar selection intensities.
