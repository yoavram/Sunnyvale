...
Comparing our results with previous results from a similar model with a fixed rate~\citep{Ram2018}, we find that a protected polymorphism can be maintained only if fluctuations in the rate of vertical transmission are small, and that stronger selection permits larger fluctuations while still maintaining a protected polymorphism. 
...
When fluctuations are stochastic, we find that if vertical transmission covaries with selection, the phenotype that is more likely to be vertically transmitted specifically when it is favored will likely fix in the population, eventually.
However, if transmission and selection are independent, then a polymorphism can be maintained if selection doesn't, on average, favor one phenotype over the other.
