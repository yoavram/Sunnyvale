...
For the simple case of unconditional migration $m_1=m_2=m<1/2$ \citep{McPeek1992} after symmetric selection $w_1=1/w_2=w>1$, the recursions~\eqref{eq:migration_model} become
...
\end{aligned}
\end{equation}
First, the discriminant $\Delta = b^2-4ac$ is positive for any real $m$, because, when writing it as a quadratic polynomial in $m$, $\Delta(m)=\frac{1}{w^{2}} \big(m^{2} (w^2+1)^2 - 2 m (w^2-1)^2 + (w^2-1)^2\big)$, this polynomial is positive at $m=0$ and $m=1/2$ and has a negative discriminant $-16w^2(w^2-1)^2$.
...
\end{aligned}\end{equation}
In addition, if we write $\Delta$ as a polynomial in $m$, its discriminant is negative -- $- 16 (w + \rho)^2 (w - 1)^2 \big(w (1 - \rho) + \rho \big) / w^2$, and therefore the discriminant $\Delta$ is positive for any $0 \le m \le 1/2$ and $f(\lambda)$ has two real roots. \\
