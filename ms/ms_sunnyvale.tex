...
\begin{aligned}
\gamma_1 x_1' &= (1-m) x_1^s + \tau m x_2^s, \quad \gamma_1 = (1-m) + \tau m, \\
\gamma_2 x_2' &= m x_1^s + \tau (1-m) x_2^s, \quad \gamma_2 = m + \tau (1-m),
\end{aligned}
\end{equation}
where $0 \le m \le 1/2$ is the migration rate, i.e. the fraction of the population that is replaced by migrants; $\tau>0$ is the relative size of deme 2 compared to deme 1 (at least where migration is concerned); and $\gamma_j$ is a normalizing factor.
This is a \emph{two-deme stepping-stone migration scheme}~\citep[][eqs.~2.15-16]{Karlin1982}.

%todo
%do simulations to find stable polymorphisms
%how do m1/m2 and rho1/rho2 balance each other in terms of polymorphism
\paragraph{Symmetric migration and selection.}
 
In the case of symmetric migration $\tau=1$ occurring after symmetric selection $w_1=1/w_2=w>1$, the recursions~\eqref{eq:migration_model_selection_transmission}~and~\eqref{eq:migration_model_migration} become
