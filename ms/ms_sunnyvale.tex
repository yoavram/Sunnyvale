...
For example, \citet{Webster2008} have demonstrated that minnows (small freshwater shoaling fish), increase their reliance on social learning when predation risk increases. %[MWF-NOTE: non-parental: adults in the parental generation]
...
\paragraph{Unconditional migration and symmetric selection.}%[MWF: Why call it unconditional?]
 
For the simple case of unconditional migration $m_1=m_2=m$ \citep{McPeek1992} after symmetric selection $w_1=1/w_2=w>1$, the recursions~\eqref{eq:migration_model} become
...
However, for $\rho=0$ we have $a+b+c = \frac{m}{w}(w-1)>0$, in which case the stability is determined by $2a+b=\frac{m(w+1)+w-1}{w}$, the derivative of the polynomial at $\lambda=1$, which is positive, and therefore both eigenvalues occur in $(0,1)$ and fixation is stable. %[MWF: Are the roots real?]
...
m > \hat m = \frac{\rho (w-1)}{\rho (w-1) +1-\rho}.
\end{equation}
In addition, $2a+b$ is linear in $m$ and equal to 2 when $m=1$; it is equal to zero when $m=(w-1)(w\rho-1)/(w(w-1)\rho+w+1) < \hat m$, and therefore \eqref{eq:migration_model_unconditional_symmetric_condition_m} 
