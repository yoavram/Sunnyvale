...
For the case of symmetric migration $m_1=m_2=m$ ($0\le m \le1/2$) occurring after symmetric selection $w_1=1/w_2=w>1$, the recursions~\eqref{eq:migration_model} become
...
Second, $a>0$, $b<0$, and $c>0$ so $f_0$ is positive with a negative derivative at $x=0$ and has a positive derivative at infinity.
...
$f_1(1)>0$ occurs if $\rho < \frac{m}{m+(1-m)(w-1)}$.
$f_1'(1)>0$ occurs if $\rho < \frac{m(w+1)+w-1}{(1-m)w(w-1)}$, which is larger than $\frac{m}{m+(1-m)(w-1)}$ for any $w>1$ and $0 \le m \le 1/2$ (the difference is a quadratic in $w$ with a negative discriminant $2m-1$ and negative value $-1/2$ at $w=2$, $m=0$). 
Therefore, fixation of $A$ is stable iff $\rho < \frac{m}{m+(1-m)(w-1)}$ \eqref{eq:migration_model_unconditional_symmetric_condition_rho}, or, by rearranging this inequality, we can also get an expression for $m$ \eqref{eq:migration_model_unconditional_symmetric_condition_m}.
