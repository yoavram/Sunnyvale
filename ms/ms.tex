...
For the simplified case of unconditional migration $m_1=m_2=m$ \citep{McPeek1992} and symmetric selection $w_1=1/w_2=w>1$, the equilibrium $(x_1^*, x_2^*)$ solves
...
With only oblique transmission ($\rho_1=\rho_2=0$), the solutions to \eqref{eq:migration_model_unconditional_symmetric} are the neutral equilibria $(x^*,x^*)$ for any $0 \le x^* \le 1$.
With only vertical transmission ($\rho_1=\rho_2=1$), the solutions to \eqref{eq:migration_model_unconditional_symmetric} are $(0,0)$, $(1,1)$ and the polymorphic solution
\begin{equation}
\begin{aligned} \label{eq:migration_model_unconditional_symmetric_solution}
x_1^* &= \frac{w-1-m(w+1) + \sqrt{\Delta}}{2(w-1)}, \\
x_2^* &= 1-x_1^*,
\end{aligned}
\end{equation}
where $\Delta = m^2(w+1)^2+(1-2m)(w-1)^2$.
Examining the stability of these equilibria, the characteristic polynomial of the Jacobian is the same for both fixation equilibria $(0,0)$ and $(1,1)$ and is given by $a \lambda^2 + b \lambda + c$ with
\begin{equation}
\begin{aligned}
a &= 1, \quad
...
\end{aligned}
\end{equation}
Since $a>0$, $b<0$, and $c>0$, and because $a+b+c=-\frac{1-m}{w}(w-1)^2<0$, the leading eigenvalue is positive, both equilibria are unstable, and the protected polymorphism exists and is given by~\eqref{eq:migration_model_unconditional_symmetric_solution}.

With only vertical transmission in one deme ($\rho_1=1$) and a combination of both vertical and oblique transmission in the other deme ($\rho_2=\rho$), the stability of $(0,0)$ is determined by a characteristic polynomial with coefficients
\begin{equation}
\begin{aligned}
a &= 1, \quad
...
Regarding the stability of $(1,1)$, which is determined by a characteristic polynomial  with coefficients
\begin{equation}
\begin{aligned}
a &= 1, \quad
...
However, for $\rho=0$ we have $a+b+c = \frac{m}{w}(w-1)>0$, in which case the stability is determined by $2a+b=\frac{m(w+1)+w-1}{w}$, the derivative of the polynomial at $\lambda=1$, which is positive, and therefore both eigenvalues occur at $(0,1)$ and the fixation is stable.
We can also determine the rate $\hat \rho$ for which $a+b+c=0$, such that for
\begin{equation} \label{eq:migration_model_unconditional_symmetric_condition_rho}
\rho < \hat \rho = \frac{m}{m+(1-m)(w-1)}
\end{equation}
the fixation $(1,1)$ is stable if and only if $2a+b>0$.
...
A similar condition can be found for $m$: the value of $a+b+c$ at $m=0$ and $m=1$ is $-\frac{\rho(w-1)^2}{w}<0$ and $\frac{(1-\rho)(w-1)}{w}>0$, respectively, and $a+b+c$ is linear in $m$, so that we have $a+b+c>0$ when 
...
\autoref{fig:migration_rho} and \autoref{fig:migration_m} show the stable frequencies of phenotype $A$ (i.e. the frequencies that solve \eqref{eq:migration_model_unconditional_symmetric_solution}) and the stable population mean fitnesses in the two demes with symmetric selection such that $A$ is favored in deme 1 and $B$ is favored in deme 2 with similar intensities (see figure legend).
...
The polymorphism $(x_1^*, x_2^*)$ is protected when transmission rates are equal, but not when transmission rates differ enough and migration is strong enough, and phenotype $A$ fixes. 
...
The shaded area denotes stables fixation of phenotype $A$ according to~\eqref{eq:migration_model_unconditional_symmetric_condition_rho}.
}
...
The shaded area denotes stables fixation of phenotype $A$ according to~\eqref{eq:migration_model_unconditional_symmetric_condition_m}.
}
