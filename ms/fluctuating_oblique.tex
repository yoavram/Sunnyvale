...
Second, 
%if $s_t$ is symmetric around zero ($E[s_t]=0$), and $\rho_t$ and $s_t$ are negatively correlated, such that vertical transmission is more likely when $B$ is favored, then $E[-\rho_t s_t]<0$ and therefore fixation of $B$ is stochastically locally stable.
let $s_t \sim U(-1,1)$ and $\rho_t \sim \beta(1+s, 1)$ (\autoref{fig:beta}), such that the covariance of $s_t$ and $\rho_t$ is positive ($cov(s_t, \rho_t) \approx 0.22$), that is, vertical transmission is more likely when $A$ if favored (when $s_t>0$) and oblique transmission is more  likely when $B$ is favored (when $s_t<0$), then ($Beta$ being the beta function), 
\begin{equation}
...
\textbf{(A)} Distribution of $\rho_t \sim Beta(1+s_t, 1)$ for different values of $s_t$.
It can be seen that for $s_t>0$ vertical transmission is more likely, and for $s_t<0$ oblique transmission is more likely.
\textbf{(B)} The compound distribution of $\rho_t \sim Beta(1+s_t, 1)$ for $s_t \sim U(-1,1)$, demonstrating that, overall, oblique transmission is more likely than vertical transmission.
\textbf{(C)} The joint distribution of $\rho_t$ and $s_t$ demonstrates a positive correlation.
}
\label{fig:beta}
\end{figure*}
