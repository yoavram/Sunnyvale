...
% TODO explain how I calculated 4 and -4
...
Here, $s_t=0.05$ and $\rho_t=\rho_1$ with probability $p=0.505$ and $s_t=-0.05$ and $\rho_t=\rho_2$ with probability $1-p=0.495$, a combination that allows a stochastic polymorphism with a fixed vertical transmission rate $\rho=0.1$ in~\citet[Fig.~2]{Ram2018}.}
...
The mean $\mu(x)$ and variance $\sigma^2(x)$ of the change in the frequency of $A$ in the case of a cycling environment $AkBl$, where $k+l=n$, are
\begin{equation}
\mu(x) = S_n x(1-x)
\quad \text{and} \quad
\sigma^2(x) = n x (1-x),
\end{equation}
where $S_n = \sum_{t=1}^{n}{\rho_t s_t}$, which the sum of selection coefficients in favor of phenotype $A$ over $n$ generations weighted by the vertical transmission rate $\rho_t$.
...
Comparing our results with previous results from a similar model with a fixed rate~\citep{Ram2018}, we find that $\ldots$ % TODO
